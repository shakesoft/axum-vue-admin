...
use crate::common::entities::{ENTITY_ATTR_NAME, ENTITY_TYPE_POLICY};
use crate::common::function::reload_policies_and_schema;
...
use cedar_policy::{
    Entities, Entity, EntityId, EntityTypeName, EntityUid, Policy, RestrictedExpression,
};
...
impl CedarPolicyService {
    ...
    async fn get_policy_entities(&self, policy_uuid: &str) -> Result<Entities, AppError> {
        ...
        let entities: Entities = match policy {
            ...
            None => Err(not_found!("Cedar policy not found"))?,
        };
    ...
    pub async fn get_policy(
        ...
        let result = cedar_policy_set::Entity::find()
            ...
            .ok_or(not_found!(format!(
                "Policy with UUID '{}' not found",
                policy_uuid
            )))?;

        let (policy, user_opt) = result;
        let created_user = user_opt
            .map(|u| u.username)
    ...
    pub async fn create_policy(
        ...
        let annotation = policy
            .annotation("annotation")
            .ok_or(bad_request!("Policy must have an 'annotation'"))?
            .to_string();
        ...
        {
            return Err(conflict!(
                "A policy with the exact same content already exists."
            ));
        }
        ...
        let (email, user_id) = users::Entity::find()
            ...
            .one(self.app_state.db.as_ref())
            .await?
            .ok_or(not_found!("User {} not found", current_user.sub))?;
        ...
        let new_model = new_policy_model.insert(self.app_state.db.as_ref()).await?;
    ...
    pub async fn update_policy(
        ...
        let annotation = policy
            .annotation("annotation")
            .ok_or(bad_request!("Missing annotation for policy"))?
            .to_string();
        let effect = policy.effect().to_string();
        let policy_hash = Self::hash_policy_content(&policy.to_string())?;
        ...
        let new_model = policy_model.update(self.app_state.db.as_ref()).await?;
