...
impl AppState {
    pub async fn new(config: &AppConfig) -> Result<Self, AppError> {
        ...
        let schema = load_active_schema(arc_db.clone()).await?;

        let cache_service = Arc::new(CacheService::new(redis.clone(), schema.clone()));
        let auth_service = Arc::new(CedarAuthService::new(cache_service.clone(), schema.clone()));

        let initial_policies = load_active_policies_and_templates(arc_db.clone()).await?;
        auth_service
            .update_policies_and_templates_in_cache(&initial_policies)
            .await?;
        info!("已成功加载并缓存策略和模板。");

        let initial_link_records = load_all_template_links(arc_db.clone()).await?;
        auth_service
            .update_template_link_records_in_cache(&initial_link_records)
            .await?;
        info!("成功加载并缓存 {} 个模板链接。", initial_link_records.len());

        let policy_link_manager =
            Arc::new(PolicyLinkManager::new(arc_db.clone(), auth_service.clone()));
